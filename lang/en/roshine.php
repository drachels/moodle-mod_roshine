<?php
// This file is part of Moodle - http://moodle.org/
//
// Moodle is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// Moodle is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with Moodle.  If not, see <http://www.gnu.org/licenses/>.

/**
 * English strings for roshine
 *
 * @package    mod_roshine
 * @copyright  2012 Jaka Luthar (jaka.luthar@gmail.com)
 * @copyright  2016 onwards AL Rachels (drachels@drachels.com)
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

defined('MOODLE_INTERNAL') || die();

$string['accuracy'] = 'Accuracy';
$string['allstring'] = 'All';
$string['average'] = 'Average';
$string['byroshine'] = 'by exercise';
$string['byuser'] = 'by user';
$string['calendarend'] = '{$a} closes';
$string['calendarstart'] = '{$a} opens';
$string['cancel'] = 'Cancel';
$string['charttitleallgrades'] = 'All Grades';
$string['charttitlemyowngrades'] = 'My Own Grades';
$string['chere'] = 'Click here to start';
$string['closebeforeopen'] = 'Could not update the Roshine. You have specified a close date before the open date.';
$string['configdateformat'] = 'This defines how dates are shown in grade reports. The default value, "M d, Y G:i" is Month, day, year and 24 hour format time. Refer to Date in the PHP manual for more examples and predefined date constants.';
$string['configpassword_desc'] = 'Whether a password is required in order to access the lesson.';
$string['configrequirepassword'] = 'Configure password';
$string['configtimelimit_desc'] = 'If a time limit is set, a warning is displayed at the beginning of the lesson and there is a countdown timer. If set to zero, then there is no time limit.';
$string['continue'] = 'Continue';
$string['continuoustype_help'] = 'If enabled, a mistyped character does not have to be corrected.';
$string['continuoustype'] = 'Continuous typing';
$string['countmistakes_help'] = 'If enabled, all keystrokes for the same mistake are added to the mistake total.';
$string['countmistakes'] = 'Count all keystrokes';
$string['countmistypedspaces_help'] = 'If enabled, a mistyped space is counted as a mistake.';
$string['countmistypedspaces'] = 'Count mistyped spaces';
$string['course_exercises_viewed'] = 'Viewed exercises';
$string['csvexport'] = 'Export to .csv';
$string['cursorcolor_colour'] = '#00ff00';
$string['cursorcolor_descr'] = 'This sets the cursor color of the next letter to type.';
$string['cursorcolor_help'] = 'This sets the color around the typing cursor. You can enter a hex color or named color. e.g. #95fc89 or green';
$string['cursorcolor_title'] = 'Cursor color';
$string['cursorcolor'] = 'Cursor color';
$string['dateformat'] = 'Default date format';
$string['defaulteditalign_help'] = 'Set text alignment while editing exercises to left, center, or right.';
$string['defaulteditalign'] = 'Edit text alignment';
$string['defaultlayout'] = 'Default keyboard layout';
$string['defaultprecision'] = 'Default typing precision %';
$string['defaulttextalign_center'] = 'center';
$string['defaulttextalign_help'] = 'Set text alignment while typing exercises to left, center, or right.';
$string['defaulttextalign_left'] = 'left';
$string['defaulttextalign_right'] = 'right';
$string['defaulttextalign_warning'] = 'NOTE: Set alignment before typing!';
$string['defaulttextalign'] = 'Text alignment';
$string['eaccess0'] = 'Any teacher';
$string['eaccess1'] = 'Teacher inside this course';
$string['eaccess2'] = 'Only me';
$string['eaddnew'] = 'Add new exercise / category';
$string['editable'] = 'Can edit';
$string['editexercises'] = 'Export / edit exercises';
$string['eeditlabel'] = 'Edit';
$string['eheading'] = 'Manage Roshine exercises';
$string['emanage'] = 'To manage exercises and categories click here';
$string['emptypassword'] = 'Password cannot be empty';
$string['ename'] = 'Exercise title';
$string['ename'] = 'Exercise: ';
$string['endlesson'] = 'End of the category / lesson';
$string['enterpassword'] = 'Please enter the password:';
$string['eremove'] = 'Remove';
$string['etext'] = 'Text';
$string['etitle'] = 'Roshine exercises';
$string['eventlessonexport'] = 'Exported lesson';
$string['examdone'] = 'Exam already taken.';
$string['excategory_help'] = 'Pick a Lesson name to use for this Roshine.';
$string['excategory'] = 'Exercises category';
$string['exercise_added'] = 'Added exercise/category';
$string['exercise_edited'] = 'Edited exercise';
$string['exercise_removed'] = 'Removed an exercise';
$string['exercise'] = 'Exercise {$a} of ';
$string['export'] = 'Export ';
$string['exportconfirm'] = 'Confirm you want to export ';
$string['failbgc_colour'] = '#FF6C6C';
$string['failbgc_descr'] = 'This sets the background color for a failing grade.';
$string['failbgc_title'] = 'Failing grade background color';
$string['fapply'] = 'Apply';
$string['fconfirm'] = 'Confirm';
$string['fcontinue'] = 'Continue';
$string['fexercise'] = 'Exercise';
$string['fexercise2'] = 'Exercise';
$string['flesson'] = 'Lesson';
$string['fmode_help'] = 'Lesson mode will present each exercise in a Lesson/Category until all are completed. Exam mode will show only the selected exercise for the exam. Once you click Save and display, you cannot change the mode.';
$string['fmode'] = 'Mode';
$string['fnewexercise'] = 'Exercise will be a part of';
$string['fnewlesson'] = 'create new lesson/category';
$string['fsecurity'] = 'Extra restrictions on attempts';
$string['fsettings'] = 'Settings';
$string['fsetup'] = 'Setup';
$string['fullhits'] = 'All hits';
$string['fullscreen'] = 'FullScreen';
$string['gexit'] = 'Back To Lession';
$string['gintro'] = 'click "Start" to play the game, type the text in the blue/pink animated box';
$string['gpause'] = 'Pause';
$string['grade_removed'] = 'Removed a grade';
$string['gradesfilename'] = 'grades.csv';
$string['gscore'] = 'Score';
$string['gstart'] = 'Start';
$string['gviewmode'] = 'View';
$string['gwait'] = 'Wait a moment';
$string['hitsperminute'] = 'Hits/minute';
$string['invalidaccess'] = 'You do not have permission to view this page.';
$string['isexamtext'] = 'Exam';
$string['kblimportadd'] = ' layout was successfully added to the database.';
$string['kblimportnotadd'] = ' layout already in the database. Nothing to do.';
$string['keybdbgc_help'] = 'This sets the background color around keyboard keys. You can enter a hex color or named color. e.g. #95fc89 or green';
$string['keybdbgc'] = 'Keyboard background color';
$string['keyboardbgc_colour'] = '#DDDDDD';
$string['keyboardbgc_descr'] = 'This sets the background color around keyboard keys.';
$string['keyboardbgc_title'] = 'Keyboard background color';
$string['keytopbgc_help'] = 'This sets the color of key tops except homerow keys. You can enter a hex color or named color. e.g. #95fc89 or green';
$string['keytopbgc'] = 'Key top color';
$string['layout_help'] = 'You can select a Keyboard layout to be shown when the Show keyboard option is set to Yes.';
$string['layout_imported'] = 'Keyboard layout imported';
$string['layout'] = 'Keyboard layout';
$string['lessioncomplete'] = 'Excercise complete!';
$string['lessioncompletealert'] = 'Congratulations! You have completed the excercise!';
$string['lessionstatistics'] = 'Excercise statistics:';
$string['lesson_export_filename'] = 'Lesson export filename';
$string['lesson_export_filenameconfig'] = 'Add GMT as part of exported lesson filenames for simple versioning system.';
$string['lesson_export'] = 'Lesson export setting';
$string['lesson_exported'] = 'Exported lesson/category';
$string['lesson_imported'] = 'Imported lesson/category';
$string['lesson_removed'] = 'Removed a lesson and its exercises';
$string['loginfail'] = 'Login failed, please try again...';
$string['lsnimport'] = 'Import lesson / KB layout';
$string['lsnimportadd'] = ' was successfully added to the database.';
$string['lsnimportnotadd'] = ' already in the database. Nothing to do.';
$string['lsnname'] = 'category name';
$string['modulename_help'] = 'The roshine module allows us to learn how to type';
$string['modulename'] = 'Roshine';
$string['modulenameplural'] = 'Roshines';
$string['nogrades'] = 'No grades yet...';
$string['normalkeytops_colour'] = '#CCCCCC';
$string['normalkeytops_descr'] = 'This sets the color of key tops except homerow keys.';
$string['normalkeytops_title'] = 'Key top color';
$string['notavailable'] = '<b>Not currently available!<br></b>';
$string['noteditablebyme'] = 'Not editable by me.';
$string['notreadyyet'] = 'Not ready yet...please try again later.';
$string['okay'] = 'Okay';
$string['options'] = 'Options';
$string['optionsalert'] = 'More options coming soon!';
$string['overview_help'] = 'Roshine is an activity for learning to type using a computer keyboard. Some lessons with multiple exercises are included and teachers, managers, and admins can add more. Lessons can be downloaded for backup or transfer to other Moodle users. Lessons can also be uploaded to augment those already installed.';
$string['overview'] = 'Overview';
$string['passbgc_colour'] = '#7FEF6C';
$string['passbgc_descr'] = 'This sets the background color for a passing grade.';
$string['passbgc_title'] = 'Passing grade background color';
$string['password'] = 'Password';
$string['passwordprotectedlesson'] = '{$a} is a password protected Roshine activity.';
$string['pluginadministration'] = 'Roshine Administration';
$string['pluginname'] = 'Roshine';
$string['practice'] = 'Practice';
$string['precision'] = 'Precision';
$string['privacy:metadata:roshine_attempts:inprogress'] = 'Status: completed = 0 or still in progress = 1. ';
$string['privacy:metadata:roshine_attempts:roshineid'] = 'ID of the Roshine activity for this attempt.';
$string['privacy:metadata:roshine_attempts:suspicion'] = 'Flag if too much time or too many mistakes.';
$string['privacy:metadata:roshine_attempts:timetaken'] = 'When the attempt was started.';
$string['privacy:metadata:roshine_attempts:userid'] = 'Userid of person making this attempt.';
$string['privacy:metadata:roshine_attempts'] = 'Stores data regarding the status of an attempt.';
$string['privacy:metadata:roshine_exercises:exercisename'] = 'The exercise name.';
$string['privacy:metadata:roshine_exercises:lesson'] = 'The lesson this exercise is in.';
$string['privacy:metadata:roshine_exercises:snumber'] = 'The sequence number in the lesson.';
$string['privacy:metadata:roshine_exercises:texttotype'] = 'The text of the exercise.';
$string['privacy:metadata:roshine_exercises'] = 'Stores exercises for each lesson.';
$string['privacy:metadata:roshine_grades:attemptid'] = 'ID number of this attempt';
$string['privacy:metadata:roshine_grades:exercise'] = 'The exercise used for this attempt';
$string['privacy:metadata:roshine_grades:fullhits'] = 'Number of keystrokes for this attempt.';
$string['privacy:metadata:roshine_grades:grade'] = 'Grade for this attempt.';
$string['privacy:metadata:roshine_grades:hitsperminute'] = 'Keystroke rate per minute.';
$string['privacy:metadata:roshine_grades:mistakes'] = 'Number of mistakes counted for this attempt.';
$string['privacy:metadata:roshine_grades:pass'] = 'Was this attempt a pass or fail?';
$string['privacy:metadata:roshine_grades:precisionfield'] = 'Typing accuracy.';
$string['privacy:metadata:roshine_grades:roshine'] = 'ID of the Roshine activity for this attempt.';
$string['privacy:metadata:roshine_grades:timeinseconds'] = 'Total elapsed time in seconds for this attempt.';
$string['privacy:metadata:roshine_grades:timetaken'] = 'When this attempt was completed?';
$string['privacy:metadata:roshine_grades:userid'] = 'Userid of person making this attempt.';
$string['privacy:metadata:roshine_grades:wpm'] = 'Rate in Words Per Minute for this attempt.';
$string['privacy:metadata:roshine_grades'] = 'Stores results of a completed exercise attempt.';
$string['privacy:metadata:roshine_lessons:authorid'] = 'The userid who added the lesson.';
$string['privacy:metadata:roshine_lessons:courseid'] = 'Courseid where the lesson was created.';
$string['privacy:metadata:roshine_lessons:editable'] = ' Editable by: only me, teacher inside this course, or any teachers.';
$string['privacy:metadata:roshine_lessons:lessonname'] = 'The name of the lesson or category.';
$string['privacy:metadata:roshine_lessons:visible'] = 'Visibilty: only me, teacher inside this course, or all teachers.';
$string['privacy:metadata:roshine_lessons'] = 'Stores lessons for Roshine.';
$string['removeall'] = 'Remove all from ';
$string['removeconfirm'] = 'You are about to completly remove ';
$string['removeexconfirm'] = 'Confirm you are about to remove an exercise from ';
$string['removekb'] = 'Remove keyboard layouts';
$string['removelsnconfirm'] = 'Confirm you are about to completly remove ';
$string['reqfield'] = 'Required field';
$string['requiredgoal_help'] = 'Enter precision required for successful completion of an exercise.';
$string['requiredgoal'] = 'Required precision';
$string['requirepassword'] = 'Require password';
$string['resetroshineall'] = 'Reset all Roshine attempts and grades';
$string['returnto'] = 'Return to {$a}';
$string['rhitspermin'] = 'Hits per minute';
$string['rmistakes'] = 'Mistakes';
$string['roshine:addinstance'] = 'Add instance';
$string['roshine:aftersetup'] = 'After setup';
$string['roshine:editall'] = 'Edit all';
$string['roshine:setup'] = 'Setup';
$string['roshine:view'] = 'View';
$string['roshine:viewgrades'] = 'View all grades';
$string['roshine:viewmygrades'] = 'View my grades';
$string['roshine'] = 'Roshine';
$string['roshineclosed'] = 'This Roshine closed on {$a}.';
$string['roshineclosetime'] = 'Close time';
$string['roshinefieldset'] = 'Custom example fieldset';
$string['roshinename_help'] = 'Because of the variety of lessons and exercises you can have, the name should make it clear which Roshine lesson or exam this acitivity is set for. Markdown syntax is supported.';
$string['roshinename'] = 'Roshine name';
$string['roshineopen'] = 'This Roshine will be open on {$a}.';
$string['roshineopentime'] = 'Open time';
$string['rprecision'] = 'Precision';
$string['rprogress'] = 'Progress';
$string['rtime'] = 'Time';
$string['sflesson'] = 'Lesson';
$string['showkeyboard_help'] = 'If enabled, a visual representation of the currently selected keyboard layout will be shown while a user is typing an exercise.';
$string['showkeyboard'] = 'Show keyboard';
$string['showkeyboardandhand'] = 'Show keyboard and hands';
$string['showrecentactivity'] = 'Show recent activity';
$string['showrecentactivityconfig'] = 'Everyone can see notifications in recent activity reports.';
$string['soundon'] = 'Turn sound on';
$string['speedwpm'] = 'Speed (WPM)';
$string['statsbgc_help'] = 'This sets the background color of the statistics bar. You can enter a hex color or named color. e.g. #95fc89 or green';
$string['statsbgc'] = 'Statistics background color';
$string['statscolor_colour'] = '#CCCCCC';
$string['statscolor_descr'] = 'This sets the background color of the statistics bar.';
$string['statscolor_title'] = 'Statistics bar background color';
$string['status'] = 'Status';
$string['student'] = 'Student';
$string['student'] = 'Student';
$string['suspicion_colour'] = '#FFFF00';
$string['suspicion_descr'] = 'This sets the marker color for a grade with suspicious results.';
$string['suspicion_title'] = 'Suspicious grade warning color';
$string['testing'] = 'Testing code';
$string['textbgc_colour'] = '#dddddd';
$string['textbgc_descr'] = 'This sets the background color of the exercise text.';
$string['textbgc_help'] = 'This sets the background color of the exercise text. You can enter a hex color or named color. e.g. #95fc89 or green';
$string['textbgc_title'] = 'Text background color';
$string['textbgc'] = 'Text background color';
$string['texterrorcolor_colour'] = '#ff9999';
$string['texterrorcolor_descr'] = 'This sets the background color of a mistyped letter.';
$string['texterrorcolor_help'] = 'This sets the background color of mistyped text. You can enter a hex color or named color. e.g. #95fc89 or green';
$string['texterrorcolor_title'] = 'Text error color';
$string['texterrorcolor'] = 'Text error color';
$string['timeinseconds'] = 'Elapsed time';
$string['timeinseconds'] = 'Time';
$string['timeisup'] = 'Time is up';
$string['timelimit_help'] = 'If enabled, a warning about the time limit is displayed at the beginning of the exercise and a countdown timer is displayed. Letters typed after time is elapsed aren\'t counted';
$string['timelimit'] = 'Time limit';
$string['timetaken'] = 'Completed';
$string['timetaken'] = 'Time taken';
$string['usepassword_help'] = 'If enabled, a password is required in order to access the lesson.';
$string['usepassword'] = 'Password protected lesson';
$string['vaccess0'] = 'All teachers';
$string['vaccess1'] = 'Teacher inside this course';
$string['vaccess2'] = 'Only me';
$string['viewgrades'] = 'View all grades';
$string['viewmygrades'] = 'View my grades';
$string['visibility'] = 'Category visible to';
$string['vmistakes'] = 'Mistakes';
$string['vtime'] = 'Time';
$string['wpm'] = 'WPM';
$string['xaxislabel'] = 'Range'; 




